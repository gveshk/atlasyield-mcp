...
export const checkRouteSurvivalDescription =
  ...
  'UNPRICEABLE). A vault that was never screened returns screened:false with no verdict — ' +
  'never a fabricated one. Call this BEFORE moving money. Read-only.';
  ...
  switch (d.verdict) {
    ...
    default:
      ...
      return { blocking: true, safeToEnter: false,
        answer: `${where}: ${d.verdict}. Do not deposit. The round-trip screen on ${d.checkedAt} could not get value back out (${pct} retained). A deposit here can look successful and still be a loss.` };
  }
}
